 ...
 * 挖沟作业设置五步进度条与各页跳转。
 * <p>
 * Step1 沟型 → Step2 A点 → Step3 B点 → 纵波 → 横波 → 作业前检查。
 * 底部进度条：第 4 段横波、第 5 段检查；纵波通过 B 点页「下一步」进入。
 */
public final class DitchStepNavigation {
    ...
    public static final int STEP_WORK = 4;
    public static final int STEP_SIDE = 5;
    public static final int STEP_PRECHECK = 6;
    ...
    public static Class<? extends Activity> activityClassForStep(int step) {
        switch (step) {
            ...
            case STEP_WORK:
                return DitchWorkSettingActivity.class;
            case STEP_SIDE:
    ...
    /** 底部进度条第 index 段（1..5）对应的逻辑步骤。第 4 段横波、第 5 段检查。 */
    ...
    /** 当前页在底部进度条上高亮的段（1..5）。纵波页高亮第 3 段（B 点已完成）。 */
    ...
    public static int logicalStepForActivity(Activity activity) {
        ...
        if (activity instanceof DitchWorkSettingActivity) return STEP_WORK;
